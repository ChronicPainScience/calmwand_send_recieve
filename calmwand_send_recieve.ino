...
void setup() {
  ...
  BLE.setLocalName("My ClamWand");  // set advertised local name and service UUID:
...
void sendFileContent(const String &filename) {
  ...
  if (!SD.exists(filename)) {
    Serial.println("File does not exist. Cannot send contents.");
    return;
  }
  ...
  while (f.available()) {
    ...
    if (line.length() > 0) {
      ...
      delay(100);  // allow central to process
    }
  }
  f.close();

  // Finally, send “EOF” so the central knows we’re done
  Serial.println("Done sending file content. Sending \"EOF\".");
...
void loop() {
  ...
  if (fileListRequestChar.written()) {
    ...
    if (cmd == "GETLIST") {
      // Open root and send each filename by setting the value (which triggers a notification)
      File root = SD.open("/");
      File entry = root.openNextFile();
      while (entry) {
        if (!entry.isDirectory()) {
          String fname = String(entry.name());    // e.g. "data0.txt"
          fileNameChar.setValue(fname);
          // no fileNameChar.notify(); needed
          delay(50);  // small pause so the central can keep up
        }
        entry.close();
      ...
      // Finally send “END” (again, setValue will notify)
      fileNameChar.setValue("END");
      // no fileNameChar.notify() here either
    }
  }
  if (fileContentRequestChar.written()) {
    String cmd = fileContentRequestChar.value();  // e.g. "GETFILE:data0.txt"
    Serial.print("Received file request: \"");
    Serial.print(cmd);
    Serial.println("\"");
    if (cmd.startsWith("GETFILE:")) {
      ...
      Serial.println("⚠️ Unknown FILE command. Ignoring.");
    }
  }
